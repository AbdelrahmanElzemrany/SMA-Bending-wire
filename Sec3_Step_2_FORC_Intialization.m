%Just Open FORC_Experiment.slx don't run it in simulink
% FIRST-ORDER REVERSAL CURVES (FORC) DATA GENERATION ONLY

clear; clc; close all;

% --- 1. CONFIGURATION & GRID DEFINITION ---
model_name = 'Step_1_FORC_Experiment'; % Replace with your exact .slx file name (without .slx)
N = 50;                              % Number of reversal curves to collect

T_max = 320;                         % Maximum temperature (Austenite state, 100% stiff)
T_min = 255;                         % Minimum temperature (Martensite state, 100% flexible)
...
total_ramp_time = 10.0;    % 10 seconds to cool down, 10 seconds to heat up
...
for i = 1:N
    ...
    simOut = sim(model_name, 'StopTime', '20.0');
    ...
    % --- Filter for data recorded during the heating phase (Time >= 10.0s) ---
